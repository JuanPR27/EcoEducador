...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Animación del header al hacer clic
  ...
  contenedores.forEach(contenedor => {
      ...
      contenedor.addEventListener('drop', function(e) {
          ...
          if (tipoResiduo === tipoContenedor) {
              ...
              const residuo = residuosArray.find(r => r.dataset.tipo === tipoResiduo && r.style.display === 'none');
              if (residuo) {
              ...
              const todosClasificados = Array.from(residuos).every(r => r.style.display === 'none');
              if (todosClasificados) {
                  mensaje.textContent = '¡Felicidades! Has clasificado todos los residuos correctamente.';
...
function setupCalculadoras() {
  ...
  document.getElementById('calcular-agua').addEventListener('click', function() {
      const duchasPorSemana = parseInt(document.getElementById('duchas').value) || 0;
      const minutosPorDucha = parseInt(document.getElementById('tiempo-ducha').value) || 0;
      ...
      resultado.innerHTML = `
          ...
          <p>¡Eso equivale a ${Math.round(ahorroAnual/1000)} metros cúbicos al año!</p>
          ${!usaReductor ? '<p>Consejo: Instala un reductor de caudal para ahorrar agua.</p>' : ''}
      `;
      resultado.style.display = 'block';
  });
  
  // Calculadora de energía
  document.getElementById('calcular-energia').addEventListener('click', function() {
      const aparato = document.getElementById('aparato').value;
      const horasUso = parseFloat(document.getElementById('horas-uso').value) || 0;
      ...
      resultado.innerHTML = `
          <h4>Resultados:</h4>
          <p>Consumo diario: <strong>${consumoDiario.toFixed(2)} kWh</strong></p>
          <p>Consumo mensual: <strong>${consumoMensual.toFixed(2)} kWh</strong></p>
          <p>Costo aproximado: <strong>$${costoMensual.toFixed(2)}/mes</strong></p>
          ${aparato === 'foco' ? 
              `<p>Con un LED ahorrarías aproximadamente $${(costoMensual * 0.83).toFixed(2)}/mes (83% menos)</p>` : 
              ''}
